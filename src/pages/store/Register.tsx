...
function Register() {
    ...
    async function handleRegister(e) {
        e.preventDefault()
        try {
            const url = `${process.env.NEXT_PUBLIC_API_URL}/store/register`
        ...
        <div className='bg-brandprimary px-4 py-20 h-[100vh]'>
            <div className='text-base font-medium text-primary flex flex-col max-w-2xl mx-auto'>
                ...
                <div className='p-10 bg-white flex flex-col gap-8 rounded-b-lg'>
                    ...
                    <div className='text-center mt-4'>Already have an account? <Link href={`/store/login`} className='text-info font-bold'>Log in</Link></div>
                </div>
            </div>
        </div>
    )
